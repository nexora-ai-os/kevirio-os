...
export default async function handler(req, res) {
  ...
  if (body.action === "affiliateResearchGenerate") {
    const verified = await resolveVerifiedOwnerWorkspaceContext(req, body.workspaceId);
    if (!verified.ok) return res.status(403).json(normalizedApiFailure({ reasonCode: verified.reasonCode }));
    if (body.paidAi === true || body.externalExecution === true) return res.status(403).json({ok:false,reasonCode:"RESEARCH_POLICY_DENIED",paidAiJpy:0,externalExecution:"LOCKED"});
    const client=createSupabaseServerClient();
    if(!client)return res.status(503).json({ok:false,reasonCode:"RESEARCH_SERVER_UNAVAILABLE",paidAiJpy:0,externalExecution:"LOCKED"});
    const programResult=await client.from("affiliate_program_master").select("id,program_name,advertiser_name,asp_name,category,reward_summary,epc,approval_rate,revisit_window_days,confirmation_days,conversion_conditions,rejection_conditions,listing_policy,listing_ng_words_verification_status,source_type,source_verified_at,source_notes,updated_at").eq("id",body.programId).eq("workspace_id",body.workspaceId).maybeSingle();
    if(programResult.error||!programResult.data)return res.status(404).json({ok:false,reasonCode:"AFFILIATE_PROGRAM_CONTEXT_NOT_FOUND",paidAiJpy:0,externalExecution:"LOCKED"});
    const program=programResult.data,required=[!program.reward_summary&&"REWARD_SUMMARY",!program.conversion_conditions&&"CONVERSION_CONDITIONS",!program.rejection_conditions&&"REJECTION_CONDITIONS",program.listing_ng_words_verification_status!=="CONFIRMED"&&"LISTING_VERIFICATION",!program.source_verified_at&&"SOURCE_VERIFIED_AT"].filter(Boolean);
    if(required.length)return res.status(409).json({ok:false,reasonCode:"AFFILIATE_RESEARCH_NOT_READY",missing:required,paidAiJpy:0,externalExecution:"LOCKED"});
    let sourceUrl;try{const parsed=new URL(String(program.source_notes||""));if(parsed.protocol!=="https:"||parsed.username||parsed.password)throw new Error();sourceUrl=parsed.toString()}catch{return res.status(409).json({ok:false,reasonCode:"AFFILIATE_RESEARCH_SOURCE_URL_REQUIRED",paidAiJpy:0,externalExecution:"LOCKED"})}
    const boundedContext=JSON.stringify({programId:program.id,programName:program.program_name,advertiserName:program.advertiser_name,aspName:program.asp_name,category:program.category,rewardSummary:program.reward_summary,epc:program.epc??"UNKNOWN",approvalRate:program.approval_rate??"UNKNOWN",revisitWindowDays:program.revisit_window_days??"UNKNOWN",confirmationDays:program.confirmation_days??"UNKNOWN",conversionConditions:program.conversion_conditions,rejectionConditions:program.rejection_conditions,listingPolicy:program.listing_policy,listingVerificationStatus:program.listing_ng_words_verification_status,sourceVerifiedAt:program.source_verified_at});
    const prompt="このAffiliate Programのcanonical dataだけを使い、実務Researchとして市場適合性、訴求機会、主要リスク、次に行うL0-L2内部準備を日本語で簡潔に分析してください。UnknownはUnknownのまま扱い、事実と推論を分離し、EvidenceやActualと断定しないでください。";
    const generated=await executeGeminiFreeRequest({text:prompt,boundedContext,workspaceId:body.workspaceId,feature:"affiliate-research",explicitOwnerAction:true},{credential:process.env.GEMINI_API_KEY});
    if(!generated.ok){const statusCode=generated.reasonCode==="GEMINI_QUOTA_EXHAUSTED"?429:generated.reasonCode==="PROVIDER_CREDENTIAL_REQUIRED"?503:502;return res.status(statusCode).json(generated)}
