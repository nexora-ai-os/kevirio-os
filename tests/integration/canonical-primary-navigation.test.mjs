...
const approvals=readFileSync(new URL("../../src/components/CanonicalApprovals.jsx",import.meta.url),"utf8");
test("primary Approval route uses the canonical Production repository workflow",()=>{assert.match(app,/approval:\s*<CanonicalApprovals ownerSupabaseClient=\{ownerSupabaseClient\}/);assert.match(approvals,/createRevenueRepository/);assert.doesNotMatch(app,/import ApprovalCenter/);});
