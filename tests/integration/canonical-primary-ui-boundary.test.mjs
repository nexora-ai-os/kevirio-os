...
test("primary canonical screens exclude the legacy floating assistant",()=>assert.doesNotMatch(app,/FloatingAssistant|pendingApprovals/));
test("zero revenue records render as unregistered rather than confirmed zero",()=>{assert.match(revenue,/hasActual \? <Money value=\{totalActual\}/);assert.doesNotMatch(revenue,/hasActual \? <Money value=\{0\}/);});
