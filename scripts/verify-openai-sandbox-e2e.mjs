...
const providerGatewaySource = readFileSync(new URL("../server/openAIProviderGateway.js", import.meta.url), "utf8");
...
check("public route requires verified server context", () => assert.ok(apiSource.includes("resolveVerifiedOwnerContext") && providerGatewaySource.includes("ownerVerified")));
check("public route enters the common platform gateway", () => assert.ok(apiSource.includes("executeOpenAIProviderGateway") && providerGatewaySource.includes("executeProviderPlatformRequest")));
...
console.log(`OpenAI sandbox E2E verification: ${passed}/14 passed`);
