...
const configuredBudgetPolicy={...OPENAI_SANDBOX_BUDGET_POLICY,maxEstimatedCostUsd:.03,monthlySandboxLimitUsd:1};
const gates = (store, overrides = {}) => ({ ownerAuthenticated: true, featureEnabled: true, usageStore: store, credential: "test-only", budgetPolicy:configuredBudgetPolicy, ...overrides });
...
await check("zero-cost fallback blocks request", () => assert.equal(evaluateCost(valid, 0).allowed, false));
await check("explicit test budget permits mocked request", () => assert.equal(evaluateCost(valid, 0, configuredBudgetPolicy).allowed, true));
...
console.log(`OpenAI sandbox adapter verification: ${passed}/29 passed`);
