...
do $$ begin
  ...
  if to_regclass('public.affiliate_cycle_publications') is not null or to_regclass('public.affiliate_cycle_performance') is not null
    or to_regclass('public.affiliate_revenue_candidates') is not null or to_regclass('public.affiliate_revenue_evidence') is not null or to_regclass('public.affiliate_actual_revenue_extensions') is not null
...
create table public.affiliate_actual_revenue_extensions(
 id uuid primary key default extensions.gen_random_uuid(),workspace_id uuid not null,owner_user_id uuid not null,
 candidate_id uuid not null,evidence_id uuid not null,affiliate_program_id uuid not null,content_id uuid not null,
 currency text not null check(currency~'^[A-Z]{3}$'),gross_amount_minor bigint not null check(gross_amount_minor>=0),cost_amount_minor bigint not null default 0 check(cost_amount_minor>=0),net_amount_minor bigint generated always as(gross_amount_minor-cost_amount_minor) stored,
 recognized_at timestamptz not null,verification_method text not null default 'M032_AFFILIATE_EVIDENCE',verified_by uuid not null,provenance jsonb not null check(jsonb_typeof(provenance)='object' and provenance<>'{}'),idempotency_key text not null check(idempotency_key~'^[A-Za-z0-9:_-]{8,180}$'),created_at timestamptz not null default clock_timestamp(),
 foreign key(owner_user_id,workspace_id) references public.account_personal_workspaces(user_id,workspace_id) on delete restrict,
 foreign key(candidate_id,workspace_id,owner_user_id) references public.affiliate_revenue_candidates(id,workspace_id,owner_user_id) on delete restrict,
 foreign key(evidence_id) references public.affiliate_revenue_evidence(id) on delete restrict,
 foreign key(affiliate_program_id) references public.affiliate_program_master(id) on delete restrict,
 foreign key(content_id) references public.personal_operational_records(id) on delete restrict,
 unique(candidate_id),unique(evidence_id),unique(owner_user_id,idempotency_key),unique(id,workspace_id,owner_user_id)
);
alter table public.affiliate_revenue_candidates add foreign key(actual_revenue_record_id) references public.affiliate_actual_revenue_extensions(id) on delete restrict;
...
create index affiliate_actual_program_idx on public.affiliate_actual_revenue_extensions(owner_user_id,affiliate_program_id,recognized_at desc);
...
begin
 ot:=case tg_table_name when 'affiliate_cycle_publications' then 'AFFILIATE_PUBLICATION' when 'affiliate_cycle_performance' then 'AFFILIATE_PERFORMANCE' when 'affiliate_revenue_candidates' then 'AFFILIATE_REVENUE_CANDIDATE' when 'affiliate_revenue_evidence' then 'AFFILIATE_REVENUE_EVIDENCE' else 'AFFILIATE_ACTUAL_REVENUE' end;
...
create trigger affiliate_actual_audit after insert on public.affiliate_actual_revenue_extensions for each row execute function public.m032_audit_cycle_change();

do $$ declare t text; begin foreach t in array array['affiliate_cycle_publications','affiliate_cycle_performance','affiliate_revenue_candidates','affiliate_revenue_evidence','affiliate_actual_revenue_extensions'] loop execute format('alter table public.%I enable row level security',t);execute format('alter table public.%I force row level security',t);execute format('revoke all on public.%I from public,anon,authenticated',t);execute format('grant select on public.%I to authenticated',t);execute format('grant select,insert,update on public.%I to service_role',t);execute format('create policy %I on public.%I for select to authenticated using(owner_user_id=auth.uid() and workspace_id=public.resolve_personal_workspace())',t||'_owner_read',t);end loop;end $$;
