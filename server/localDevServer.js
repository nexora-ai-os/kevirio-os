...
export async function createLocalDevServer(options = {}) {
  ...
  const server = createHttpServer(async (req, res) => {
    ...
    if (!["/api/ai","/api/provider","/api/employee","/api/members"].includes(pathname)) { vite.middlewares(req, res, () => sendJson(res, 404, { ok: false, status: "blocked", reasonCode: "NOT_FOUND" })); return; }
    ...
    try {
      ...
      const selected=pathname==="/api/provider"?(options.providerHandler||providerHandler):pathname==="/api/employee"?(options.employeeHandler||employeeHandler):pathname==="/api/members"?(options.membersHandler||membersHandler):handler;await selected({ method:req.method,headers:req.headers,body },createServerlessResponseAdapter(res));
    } catch (error) {
