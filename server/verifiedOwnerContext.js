...
export async function resolveVerifiedOwnerContext(req, options = {}) {
  ...
  const profile = await client.from("owner_profiles").select("role,status").eq("owner_id", ownerId).maybeSingle(); if (profile?.error || profile?.data?.role !== "owner" || profile?.data?.status !== "active") return locked("OWNER_PROFILE_NOT_ACTIVE");
