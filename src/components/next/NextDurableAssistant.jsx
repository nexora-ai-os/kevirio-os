...
export default function NextDurableAssistant({ client }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const labels = { GEMINI_QUOTA_EXHAUSTED: "無料AIの利用上限に達しました。", PROVIDER_CREDENTIAL_REQUIRED: "Gemini credentialがPrivate Beta環境で利用できません。", GEMINI_MODEL_UNAVAILABLE: "指定されたGemini modelを利用できません。", GEMINI_TIMEOUT: "Gemini APIの応答が時間内に完了しませんでした。", GEMINI_PROVIDER_ERROR: "Gemini APIに接続できませんでした。", REQUEST_ORIGIN_NOT_ALLOWED: "Private Betaの認証Originを検証できませんでした。" };
      setAnswer({ ...localAnswer(text, context), messagePrefix: `${labels[error?.reasonCode] || "Geminiを利用できませんでした。"} ローカル回答へ切り替えました。`, failureReason: error?.reasonCode || "GEMINI_RUNTIME_ERROR", provider: "LOCAL", providerState: "DETERMINISTIC", cost: "FREE / ¥0", timestamp: new Date().toISOString(), basis: "Owner Personal Workspaceで現在許可された最小限の記録", feature });
