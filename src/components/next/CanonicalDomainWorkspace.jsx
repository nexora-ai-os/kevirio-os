...
export default function CanonicalDomainWorkspace({ client, type }) {
  ...
  const submit = async (event) => {
    ...
    try { await saveCanonicalDomain(client, { type, payload: payloadFor(type, new FormData(event.currentTarget)) }); event.currentTarget.reset(); setMessage("Personal Workspaceへ保存しました。"); await reload(); }
    catch (error) { setMessage(friendlyError(error, "保存できませんでした。入力と参照先を確認してください。")); }
  };
  const saveDraft = async () => {
    ...
    try {
      ...
      setMessage("下書きを端末間で再開できる状態に保存しました。"); await reload();
    } catch (error) { setMessage(friendlyError(error, "別端末で更新されています。再読み込み後、内容を確認して保存してください。")); }
  };
  ...
  return <div className="next-grid" aria-busy={state.loading}>
    <section className="next-card next-card--wide"><header><h2>{LABELS[type]}を登録</h2></header>
      <form className="next-form" onSubmit={submit}>
        ...
        <button className="next-primary" disabled={blocked || state.loading}>保存</button>
