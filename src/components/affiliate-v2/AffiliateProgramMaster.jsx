...
function AffiliateRevenueClosure({program,onLoadContent,api}){
  ...
  const publication=state.data?.publications?.[0],performance=state.data?.performance?.[0],candidate=state.data?.candidates?.[0],evidence=state.data?.evidence?.find(x=>x.candidate_id===candidate?.id),actual=state.data?.revenue?.find(x=>x.affiliate_revenue_candidate_id===candidate?.id),content=state.content,draft=content?.payload;
