...
function AffiliateContentCycle({program,strategies,onPrepare,onLoad,onSave,revenueCycle}){
  const[records,setRecords]=useState([]),[record,setRecord]=useState(null),[contentType,setContentType]=useState("THREADS"),[draft,setDraft]=useState(null),[status,setStatus]=useState("idle"),dirtyRef=useRef(false),loadRef=useRef(onLoad);
  const confirmed=strategies.find(x=>x.status==="CONFIRMED")||null;
  useEffect(()=>{loadRef.current=onLoad},[onLoad]);
  useEffect(()=>{let active=true;dirtyRef.current=false;loadRef.current?.(program.id).then(rows=>{if(!active||dirtyRef.current)return;setRecords(rows);const latest=rows[0]||null;setRecord(latest);setDraft(latest?.payload||null)}).catch(()=>{if(active)setStatus("load_failed")});return()=>{active=false}},[program.id]);
  const markDirty=updater=>{dirtyRef.current=true;setStatus("review");setDraft(updater)};
  const prepare=async()=>{if(!confirmed)return;setStatus("generating");try{const result=await onPrepare(program,confirmed.id,contentType);dirtyRef.current=true;setDraft({workflow:"AFFILIATE_REAL_CYCLE",title:result.draft.title,content_type:contentType,body:result.draft.body,cta:result.draft.cta,asset_checklist:result.draft.assetChecklist,recommended_timing:result.draft.recommendedTiming,execution_checklist:result.draft.executionChecklist,affiliate_program_id:program.id,affiliate_strategy_id:result.strategyId,affiliate_research_id:result.researchId,review_status:"OWNER_REVIEW",execution:{state:"DRAFT",platform:contentType,final_content:result.draft.body,cta:result.draft.cta,affiliate_destination_url:program.affiliateUrl||null,pr_disclosure_reminder:"広告・PR表記とASP規約をOwnerが公開前に確認",external_url:null,published_at:null},performance:{clicks:null,conversions:null,pending_reward:null,confirmed_reward:null,rejected_reward:null,truth_class:"UNKNOWN"},analytics:{recommendation:"WAITING_FOR_REAL_EXTERNAL_RESULT",truth_class:"AI_INFERENCE"},improvement:null,truth_class:"AI_OUTPUT",evidence_status:"NOT_EVIDENCE",paid_ai_jpy:0,external_execution:"LOCKED"});setStatus("review")}catch{setStatus("failed")}};
  const save=async(lifecycleStatus="DRAFT")=>{setStatus("saving");try{await onSave({record,payload:draft,lifecycleStatus});const rows=await loadRef.current(program.id);setRecords(rows);setRecord(rows[0]||null);setDraft(rows[0]?.payload||draft);dirtyRef.current=false;setStatus("saved")}catch(error){setStatus(error?.message?.includes("stale")?"stale":"failed")}};
  const set=(key,value)=>markDirty(current=>({...current,[key]:value}));
  const setExecution=(key,value)=>markDirty(current=>({...current,execution:{...current.execution,[key]:value}}));
  const setPerformance=(key,value)=>markDirty(current=>({...current,performance:{...current.performance,[key]:value===""?null:Number(value)}}));
