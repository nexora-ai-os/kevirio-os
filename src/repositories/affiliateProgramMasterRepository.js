...
export function createAffiliateProgramMasterRepository(client) {
  ...
  return {
    ...
    async updatePractical(id,input){const normalized=normalizeAffiliateProgramPracticalUpdate(input);const request={p_program_id:id,p_expected_updated_at:normalized.expectedUpdatedAt,p_expected_business_version:normalized.expectedBusinessVersion,p_changes:normalized.changes};const{data,error}=await client.rpc("update_affiliate_program_master_practical",request);if(error)throw mapRepositoryError(error,{operation:"update_practical",object:"affiliate_program_master"});return data?.[0]||data},
    async deleteIfSafe(id,{expectedUpdatedAt,expectedBusinessVersion,idempotencyKey}){const exactUpdatedAt=String(expectedUpdatedAt||"").trim();if(!id||!exactUpdatedAt||Number.isNaN(new Date(exactUpdatedAt).valueOf())||!idempotencyKey)throw new AffiliateV2Error("VALIDATION_FAILED",{operation:"delete_identity_required",object:"affiliate_program_master"});const{data,error}=await client.rpc("delete_affiliate_program_master_if_safe",{p_program_id:id,p_expected_updated_at:exactUpdatedAt,p_expected_business_version:Number(expectedBusinessVersion),p_idempotency_key:idempotencyKey});if(error)throw mapRepositoryError(error,{operation:"delete_if_safe",object:"affiliate_program_master"});return data?.[0]||data},
