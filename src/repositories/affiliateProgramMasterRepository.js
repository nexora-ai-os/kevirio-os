...
export function createAffiliateProgramMasterRepository(client) {
  ...
  return {
    ...
    async updatePractical(id,input){const normalized=normalizeAffiliateProgramPracticalUpdate(input);const{data,error}=await client.rpc("update_affiliate_program_master_practical",{p_program_id:id,p_expected_updated_at:normalized.expectedUpdatedAt,p_expected_business_version:normalized.expectedBusinessVersion,p_changes:normalized.changes});if(error)throw mapRepositoryError(error,{operation:"update_practical",object:"affiliate_program_master"});return data?.[0]||data},
