...
create table public.operational_objects(
  ...
  object_type text not null check(object_type in(
    'GOAL','STRATEGY','WORK','APPLICATION','CLIENT','CONTENT','SNS_ITEM','KNOWLEDGE',
    'IMPROVEMENT','GLOBAL_OPPORTUNITY','RESEARCH_PACKAGE','QUICK_CAPTURE'
  )),
...
create table public.research_sources(
  ...
  workspace_id uuid not null,
  owner_user_id uuid not null,
  canonical_url text not null check(canonical_url~'^https://[^[:space:]]{1,1990}$'),
...
begin
  if jsonb_typeof(coalesce(p_value,'{}'))<>'object' or length(coalesce(p_value,'{}')::text)>p_max
    or coalesce(p_value,'{}')::text~*'(api[_ -]?key|access[_ -]?token|refresh[_ -]?token|authorization|password|private[_ -]?key)'
...
declare v_actor uuid:=auth.uid();v_workspace uuid:=public.resolve_personal_workspace();v_id uuid;v_version bigint;v_title text:=public.m027_safe_text(p_title,300);
begin
  ...
  if p_object_id is null then
    ...
    values(v_workspace,v_actor,p_object_type,v_title,case when nullif(btrim(p_summary),'') is null then null else public.m027_safe_text(p_summary,4000) end,p_state,p_attention_state,p_due_at,public.m028_safe_json(p_details,50000),p_truth_class)
    returning id,version into v_id,v_version;
  else
    update public.operational_objects set title=v_title,summary=case when nullif(btrim(p_summary),'') is null then null else public.m027_safe_text(p_summary,4000) end,state=p_state,attention_state=p_attention_state,due_at=p_due_at,
  ...
  insert into public.operational_activity_events(workspace_id,owner_user_id,object_type,object_id,event_type,actor_type,actor_id,truth_class,safe_metadata)
...
begin
  ...
  if v_version is null then raise exception 'operational_draft_stale_or_not_found';end if;return v_version;
end $$;
...
begin
  ...
  insert into public.operational_activity_events(workspace_id,owner_user_id,object_type,object_id,event_type,actor_type,actor_id,truth_class)
  values(v_workspace,v_actor,v_type,p_object_id,'ARCHIVED','OWNER',v_actor::text,'OWNER_STATED');return v_version;
end $$;
...
begin
  ...
  if p_from_type='OPERATIONAL_OBJECT' and not exists(select 1 from public.operational_objects where id=p_from_id and owner_user_id=v_actor and workspace_id=v_workspace) then raise exception 'operational_link_source_denied';end if;
  if p_to_type='OPERATIONAL_OBJECT' and not exists(select 1 from public.operational_objects where id=p_to_id and owner_user_id=v_actor and workspace_id=v_workspace) then raise exception 'operational_link_target_denied';end if;
  if p_from_type='AI_THREAD' and not exists(select 1 from public.ai_conversation_threads where id=p_from_id and owner_user_id=v_actor and workspace_id=v_workspace) then raise exception 'operational_link_source_denied';end if;
  if p_to_type='AI_THREAD' and not exists(select 1 from public.ai_conversation_threads where id=p_to_id and owner_user_id=v_actor and workspace_id=v_workspace) then raise exception 'operational_link_target_denied';end if;
  if p_from_type='AFFILIATE_PROGRAM' and not exists(select 1 from public.affiliate_program_master where id=p_from_id and workspace_id=v_workspace) then raise exception 'operational_link_source_denied';end if;
  if p_to_type='AFFILIATE_PROGRAM' and not exists(select 1 from public.affiliate_program_master where id=p_to_id and workspace_id=v_workspace) then raise exception 'operational_link_target_denied';end if;
  if p_from_type not in('OPERATIONAL_OBJECT','AI_THREAD','AFFILIATE_PROGRAM') or p_to_type not in('OPERATIONAL_OBJECT','AI_THREAD','AFFILIATE_PROGRAM') then raise exception 'operational_link_type_unsupported';end if;
  ...
  on conflict(owner_user_id,from_type,from_id,to_type,to_id,relation_type) do update set provenance=excluded.provenance returning id into v_id;return v_id;
end $$;
...
begin
  ...
  insert into public.internal_action_records(workspace_id,owner_user_id,employee_id,initiating_user_id,target_type,target_id,action_type,autonomy_level,risk_class,policy_approval,payload)
  values(v_workspace,v_actor,public.m027_safe_text(p_employee_id,80),v_actor,p_target_type,p_target_id,p_action_type,p_autonomy_level,p_risk_class,p_policy_approval,public.m028_safe_json(p_payload,50000)) returning id into v_id;return v_id;
end $$;
...
begin
  ...
  update public.internal_action_records set status=case when p_success then 'COMPLETED' else 'FAILED' end,result_summary=public.m027_safe_text(p_result_summary,8000),result_truth_class=p_truth_class,version=version+1,updated_at=clock_timestamp()
...
begin
  ...
  values(v_workspace,p_owner_user_id,p_canonical_url,public.m027_safe_text(p_source_name,300),lower(btrim(p_source_domain)),p_country_code,nullif(btrim(p_region),''),p_source_type,p_reliability_class,p_cost_class,case when nullif(btrim(p_limitations),'') is null then null else public.m027_safe_text(p_limitations,2000) end,public.m028_safe_json(p_audit_metadata,12000))
...
begin
  if auth.role()<>'service_role' then raise exception 'service_role_required';end if;
  select workspace_id into v_workspace from public.account_personal_workspaces where user_id=p_owner_user_id;if not found then raise exception 'personal_workspace_required';end if;
  insert into public.provider_free_quota_states(workspace_id,owner_user_id,provider,model_or_action,cost_class,quota_state,rate_limit_state,requests_used,requests_limit,resets_at,last_error_code,fallback_state,audit_metadata)
  values(v_workspace,p_owner_user_id,lower(btrim(p_provider)),public.m027_safe_text(p_model_or_action,120),p_cost_class,p_quota_state,p_rate_limit_state,p_requests_used,p_requests_limit,p_resets_at,p_last_error_code,p_fallback_state,public.m028_safe_json(p_audit_metadata,12000))
  on conflict(owner_user_id,provider,model_or_action) do update set cost_class=excluded.cost_class,quota_state=excluded.quota_state,rate_limit_state=excluded.rate_limit_state,requests_used=excluded.requests_used,requests_limit=excluded.requests_limit,resets_at=excluded.resets_at,last_error_code=excluded.last_error_code,fallback_state=excluded.fallback_state,checked_at=clock_timestamp(),audit_metadata=excluded.audit_metadata
  returning id into v_id;return v_id;
end $$;
...
returns uuid language plpgsql security definer set search_path='' as $$
declare v_workspace uuid;v_id uuid;v_text text:=public.m027_safe_text(p_statement,8000);v_hash text;
begin
  if auth.role()<>'service_role' then raise exception 'service_role_required';end if;
  ...
  values(v_workspace,p_owner_user_id,p_source_id,p_domain,nullif(btrim(p_market),''),p_country_code,p_language_code,p_observed_at,p_freshness_expires_at,v_text,p_truth_class,p_confidence,v_hash,public.m028_safe_json(p_provenance,12000),p_supersedes_id) returning id into v_id;return v_id;
end $$;
