do $$ declare bad int;begin
 select count(*) into bad from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in('m032_owner_cycle_context','m032_audit_cycle_change','save_affiliate_cycle_publication','record_affiliate_cycle_performance','create_affiliate_revenue_candidate','attach_affiliate_revenue_evidence','confirm_affiliate_actual_revenue') and not(coalesce(p.proconfig,'{}')@>array['search_path=""']);
...
select jsonb_build_object('result','M032_NAMESPACE_COMPATIBILITY_PASS','safe_search_path',7,'extension_schema','extensions','paid_ai_jpy',0) verification;
