select jsonb_build_object('result',case when sum(v)=0 then 'M032_DATA_HEALTH_PASS' else 'FAIL' end,'violations',sum(v)) verification from(
 ...
 select count(*) from public.affiliate_actual_revenue_extensions r left join public.affiliate_revenue_candidates c on c.id=r.candidate_id where c.id is null or r.gross_amount_minor<>c.amount_minor or r.currency<>c.currency or c.status<>'CONFIRMED_ACTUAL'
)x;
