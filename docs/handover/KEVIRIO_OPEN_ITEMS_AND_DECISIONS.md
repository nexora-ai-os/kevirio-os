...
| D-02 | Authorization | Multi-user/admin/staff roles | Not Implemented beyond Owner/member foundation | Current flows are Owner-centric | Access cannot safely broaden | Role matrix and lifecycle policy | Yes | Keep Owner-only assumptions pending approval | RLS/RPC/UI tests prove least privilege for each role |
